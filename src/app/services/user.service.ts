...
export class UserService {
  ...
  //     return throwError('JWT token is missing');
    ...
    getRooms(): Observable<any[]> {
      return this.http.get<any[]>(`${this.apiUrl}/roommanages`).pipe(
        catchError(error => throwError(error))
      );
    }
    ...
    generateFavRooms(): Observable<any[]> {
      return this.http.get<any[]>(`${this.apiUrl}/generate-fav-room`).pipe(
        catchError(error => throwError(error))
      );
    }
...
// Revenue Report
getRevenueRange(startYear: number, startMonth: number, endYear: number, endMonth: number): Observable<{ revenue: { year: number, month: number, totalRevenue: number }[] }> {
  const token = localStorage.getItem(this.tokenKey); // Retrieve the token from local storage or another secure place
  if (!token) {
    throw new Error('Authentication token is missing');
  }
  const headers = new HttpHeaders().set('Authorization', `Bearer ${token}`);
  return this.http.get<{ revenue: { year: number, month: number, totalRevenue: number }[] }>(
    `${this.apiUrl}/api/report/range?startYear=${startYear}&startMonth=${startMonth}&endYear=${endYear}&endMonth=${endMonth}`,
    { headers }
  );
}
